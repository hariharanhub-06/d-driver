...
const { triggerSOS, cancelSOS, resolveSOS, getSOSAlerts, getMyActiveSOS, triggerParentSOS } = require('../controllers/sosController');

const auth = [authenticateToken, requirePasswordChanged];

router.post('/trigger', ...auth, requireRole('driver'), triggerSOS);
router.post('/parent', ...auth, requireRole('parent'), triggerParentSOS);
