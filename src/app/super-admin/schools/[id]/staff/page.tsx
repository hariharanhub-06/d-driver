...
export default function SchoolStaffPage() {
    ...
    const fetchStaff = async () => {
        ...
        setError('');
        try {
            const { data } = await api.get(`/users?role=bus_staff&school_id=${id}`);
        ...
        <div className="space-y-5">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-2.5">
                    ...
                    <h2 className="text-slate-900 dark:text-white font-bold text-lg">Bus Staff</h2>
                    <span className="text-xs bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400 rounded-full px-2.5 py-0.5 font-medium">{staff.length}</span>
                </div>
                <button
                                    ...
                                    <tr key={s.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                                        <td className="px-4 py-3">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="font-semibold text-slate-900 dark:text-white text-sm">{s.name}</span>
                                            </div>
                                        </td>
                        ...
                        {createdPassword ? (
                            <div className="p-6 space-y-4">
                                ...
                                <div className="bg-slate-50 dark:bg-slate-700/50 rounded-xl p-4 flex items-center justify-between gap-3 border border-slate-200 dark:border-slate-600">
                                    ...
                                    <button onClick={copyPassword} className="shrink-0 flex items-center gap-1.5 text-xs font-semibold text-[var(--brand)] hover:opacity-80 transition-opacity">
