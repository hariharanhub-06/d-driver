    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-slate-900 pb-20">
                ...
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-white/80 text-sm mt-0.5">
                            {user?.name || 'Bus Staff'} · Bus Conductor
                        </p>
                    </div>
                    <button onClick={logout} className="text-white/70 text-xs hover:text-white px-3 py-1.5 border border-white/30 rounded-lg transition-colors">
                        Logout
                    </button>
                </div>
                <p className="text-white/80 text-sm mt-2">
                    {currentStop ? `Stop ${currentStopIndex + 1}/${stops.length}: ${currentStop.name}` : 'No active trip'}
                </p>
            </div>
            ...
            <div className="px-4 mt-4 space-y-3">
                {filteredStudents.length === 0 && stops.length > 0 ? (
                    ...
                    filteredStudents.map(student => {
                        ...
                        return (
                            ...
                            >
                                ...
                                {preAbsent && !marked ? (
                                    <span className="bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 rounded-full px-2.5 py-0.5 text-xs font-medium">Pre-Absent</span>
                                ) : marked ? (
                                    <span className={cn(
                ...
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
                    <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
                        <div className="flex items-center justify-between p-4 border-b border-slate-100 dark:border-slate-700">
                            ...
                            <button onClick={() => { setActiveStudentId(null); setMarkingNote(''); }} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg text-slate-400 transition-colors">
                                <X className="w-5 h-5" />
                            </button>
                        </div>
                        ...
                        <div className="p-6">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <button
                                    ...
                                    className="py-4 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 rounded-2xl font-bold flex flex-col items-center gap-2 border border-red-100 dark:border-red-800 hover:bg-red-100 transition-all active:scale-95"
                                >
                                    <X size={24} />
                                    Absent
                                </button>
                            ...
                            <button
                                ...
                                className="w-full mt-4 py-2.5 text-xs font-medium text-slate-400 dark:text-slate-500 hover:text-slate-600 transition-colors"
                            >
                                Cancel
                            </button>
                        </div>
