        ...
        <div className="space-y-4 p-4">
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 p-5">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <p className="text-lg font-bold text-slate-900 dark:text-white">{user?.name || 'Parent Name'}</p>
                        <p className="text-sm text-slate-500 dark:text-slate-400">{user?.email}</p>
                        <span className="inline-flex items-center mt-1 bg-[var(--brand)]/10 text-[var(--brand)] rounded-full px-2.5 py-0.5 text-xs font-medium">Parent</span>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 p-5">
                ...
                <form onSubmit={handleChangePassword} className="space-y-4">
                    ...
                    {cpError && (
                        <div className="flex items-center gap-2 text-sm text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 rounded-xl px-4 py-2.5">
                    ...
                    <button
                        type="submit"
                        disabled={cpLoading}
                        className="w-full bg-[var(--brand)] hover:opacity-90 text-white rounded-xl py-2.5 font-semibold text-sm disabled:opacity-50 transition-all active:scale-95"
                    >
            ...
            <button
                onClick={logout}
                className="w-full flex items-center justify-center gap-2 py-2.5 text-sm font-medium text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl transition-colors border border-red-200 dark:border-red-900/30 bg-white dark:bg-slate-800"
            >
                <LogOut className="w-4 h-4" />Logout
            </button>

            <div className="text-center text-slate-400 text-[10px] font-medium uppercase tracking-widest py-4">
